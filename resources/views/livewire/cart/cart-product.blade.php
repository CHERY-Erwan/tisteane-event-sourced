...
new class extends Component
{
    ...
    public function updateItemQuantity(int $quantity, UpdateItemQuantityAction $action): void
    {
        $action(
    ...
    {
        return number_format($this->item->item->price * $this->item->quantity / 100, 2, ',', '.');
    }
}; ?>
            ...
            <div class="flex justify-between">
                <flux:button class="w-1/3 h-10 !flex !flex-row !justify-around !items-center">
                    <flux:icon
                        ...
                        wire:click="updateItemQuantity({{ $item->quantity - 1 }})"
                    />
                    {{ $item->quantity }}
                    <flux:icon
                        name="plus"
